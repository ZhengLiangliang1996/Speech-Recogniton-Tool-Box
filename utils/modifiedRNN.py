...
class LSTM_forget(Recurrent):
    ...
    def __init__(self, units,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(LSTM_forget, self).__init__(**kwargs)
        self.units = units
        self.activation = activations.get(activation)
        self.recurrent_activation = activations.get(recurrent_activation)
        self.use_bias = use_bias
    ...
    def step(self, inputs, states):
        ...
        if self.implementation == 2:
            ...
            i = self.recurrent_activation(x_i + K.dot(h_tm1 * rec_dp_mask[0],
                                                      self.recurrent_kernel_i))
            # relu before the recurrent_activation
            relu = activations.get('relu')
            fi= relu(x_f + K.dot(h_tm1 * rec_dp_mask[1],self.recurrent_kernel_f))
            f = self.recurrent_activation(fi)
            #f = np.heaviside((x_f + K.dot(h_tm1 * rec_dp_mask[1],
            #                  self.recurrent_kernel_f)), 0.5)
            c = f * c_tm1 + i * self.activation(x_c + K.dot(h_tm1 * rec_dp_mask[2],
                                                            self.recurrent_kernel_c))
            o = self.recurrent_activation(x_o + K.dot(h_tm1 * rec_dp_mask[3],
                                                      self.recurrent_kernel_o))
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(LSTM_forget, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))



class LSTMP(Recurrent):
    """Long-Short Term Memory unit - Hochreiter 1997.
