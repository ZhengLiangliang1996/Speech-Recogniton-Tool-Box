...
<<<<<<< HEAD
parser.add_argument('--epochs', type=int, default=100, help='Number of training steps')
...
<<<<<<< HEAD
            ...
            # save to txt file
            if args.mode == 'train' and not args.restore:
                np.savetxt(resultdir+'/'+str(args.num_layers)+args.mode+'_loss.csv', all_lossRate, delimiter=',')
                np.savetxt(resultdir+'/'+str(args.num_layers)+args.mode+'_char_error.csv', all_errRate, delimiter=',')
=======
                    logging_helper(model, logfile, epochLoss, epochER, mode=args.mode)     
                    all_errRate[epoch] = epochER
                    all_lossRate[epoch] = epochLoss
